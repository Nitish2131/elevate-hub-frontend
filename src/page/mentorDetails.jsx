    ...
    const fetchMentorDetails = async () => {
      try {
        ...
        if (response?.data?.mentor) {
          ...
          setServices(response.data.mentor.services || []);
        } else {
            ...
            {loading ? (
              ...
              mentor && (
                <>
                  ...
                  />
                  <h2 className="mt-4 text-3xl font-bold text-center">{mentor?.name}</h2>
                  <p className="mt-2 text-center text-gray-600">{mentor?.profile?.title}</p>
                  
                  <div className="flex justify-center mt-4">
                    {mentor?.profile?.tags?.map((tag, index) => (
                      <span key={index} className="px-2 py-1 mx-1 text-xs bg-gray-100 rounded-full">
                        {tag || "Tags"}
                      </span>
                    ))}
                  </div>
                  
                  <p className="mt-4 text-center text-gray-500">{mentor?.profile?.bio || "Mentor bio"}</p>
                  
                  <h3 className="mt-8 text-lg font-semibold text-center">Connect with me</h3>
                  <div className="flex justify-center mt-4 space-x-4">
                    <a href={mentor?.profile?.social?.linkedin || "#"} target="_blank" rel="noopener noreferrer">
                      <AiFillLinkedin className="text-3xl text-blue-600" />
                    </a>
                    <a href={mentor?.profile?.social?.github || "#"} target="_blank" rel="noopener noreferrer">
                      <AiFillGithub className="text-3xl text-gray-800" />
                    </a>
                    <a href={mentor?.profile?.social?.twitter || "#"} target="_blank" rel="noopener noreferrer">
                      <AiFillTwitterCircle className="text-3xl text-blue-400" />
                    </a>
                    <a href={mentor?.profile?.social?.facebook || "#"} target="_blank" rel="noopener noreferrer">
                      <AiFillFacebook className="text-3xl text-blue-700" />
                    </a>
                    <a href={mentor?.profile?.social?.instagram || "#"} target="_blank" rel="noopener noreferrer">
